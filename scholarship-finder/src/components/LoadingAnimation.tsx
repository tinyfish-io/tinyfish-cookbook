...
import type { AgentStatus, SearchState } from "@/types/scholarship";
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="relative inline-block">
          ...
          <div className="absolute inset-0 w-16 h-16 rounded-full border-4 border-primary/20 border-t-primary animate-spin" style={{ animationDuration: "2s" }} />
        </div>
        <div>
        ...
        {agents.length > 0 && (
          <div className="flex items-center justify-center gap-4 text-sm">
            <span className="flex items-center gap-1 text-muted-foreground">
              <Loader2 className="w-4 h-4 animate-spin text-primary" />{runningAgents.length} running
            </span>
            <span className="flex items-center gap-1 text-green-600">
              <CheckCircle className="w-4 h-4" />{completedAgents.length} done
            </span>
            {errorAgents.length > 0 && (
              <span className="flex items-center gap-1 text-red-500">
                <AlertCircle className="w-4 h-4" />{errorAgents.length} failed
              </span>
            )}
          </div>
        )}
        ...
        {expandedAgent && searchState.agents[expandedAgent]?.streamingUrl && (
          ...
          >
            ...
            >
              ...
              <iframe src={searchState.agents[expandedAgent]?.streamingUrl} className="w-full h-[60vh] border-0" title="Live preview" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Real-time results as they stream in */}
      {searchState.completedScholarships.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <CheckCircle className="w-5 h-5 text-green-500" />
            <h3 className="text-lg font-semibold">Found {searchState.completedScholarships.length} Scholarships So Far</h3>
          </div>
...
function AgentCard({ agent, isExpanded, onExpand }: { agent: AgentStatus; isExpanded: boolean; onExpand: () => void }) {
  ...
  return (
    <motion.div
      ...
      className={cn("bg-card border-2 rounded-lg overflow-hidden transition-colors", statusColors[agent.status])}
    >
      <div className="px-3 py-2 bg-muted/50 border-b border-border flex items-center justify-between">
        ...
        {agent.streamingUrl && agent.status === "running" && (
          <button onClick={onExpand} className="p-1 hover:bg-primary/10 rounded transition-colors">
      ...
      <div className="relative">
        {agent.streamingUrl && agent.status === "running" ? (
          <div className="h-32 bg-white relative">
            <iframe src={agent.streamingUrl} className="w-full h-full border-0 pointer-events-none" title={`Preview of ${agent.siteName}`} />
          ...
          <div className="h-32 bg-muted/30 flex flex-col items-center justify-center p-3">
            {agent.status === "pending" && <><Loader2 className="w-6 h-6 animate-spin text-muted-foreground mb-2" /><p className="text-xs text-muted-foreground">Waiting to start...</p></>}
            {agent.status === "complete" && <><CheckCircle className="w-6 h-6 text-green-500 mb-2" /><p className="text-xs text-center text-muted-foreground">Found {agent.scholarships?.length || 0} scholarships</p></>}
            {agent.status === "error" && <><AlertCircle className="w-6 h-6 text-red-500 mb-2" /><p className="text-xs text-center text-red-500 truncate max-w-full">{agent.error || "Failed"}</p></>}
          </div>
        )}
      </div>
      {agent.siteUrl && (
        <div className="px-3 py-2 bg-muted/30 border-t border-border">
          <a href={agent.siteUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1 truncate">
