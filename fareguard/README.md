...
- 7 agents run in parallel, one per site (Vietjet, Vietnam Airlines, Bamboo Airways, Traveloka, Baolau, 12Bay, Skyscanner VN). Each one pulls fares for both tracked routes.
...
- Routes tracked: Hanoi↔Ho Chi Minh City, Ho Chi Minh City↔Da Nang — searched for today's date (Vietnam time, UTC+7), one date per sweep, since the point of the 8-hourly cadence is watching the same date's price move, not scanning a range.
...
- TinyFish SDK (`@tiny-fish/sdk`) for the real scraping
