...
import type { StreamingPreview } from '@/lib/types';
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          {moreCount > 0 && (
            ...
            >
              {expanded ? '− Show less' : `+ Show ${moreCount} more scout${moreCount > 1 ? 's' : ''}`}
            </button>
          )}
        </div>
      </div>
        ...
        {visible.map(({ siteUrl, streamingUrl, done }) => (
          ...
          >
            ...
            <iframe
              ...
              title={`TinyFish deal scout: ${getHostname(siteUrl)}`}
              loading="eager"
            />
          </div>
        ))}
      ...
      {!expanded && moreCount > 0 && (
        <p className="text-xs text-zinc-400 text-center">
          {previews.length} scouts running in parallel —{' '}
          <button
            type="button"
