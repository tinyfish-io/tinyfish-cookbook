...
import type { AgentState, Sector } from "@/types/tender";
...
export function AgentPreviewGrid({ agents, sector }: AgentPreviewGridProps) {
  ...
  const activeCount = agents.filter((a) => a.status === "searching" || a.status === "connecting").length;

  const sortedAgents = [...agents].sort((a, b) => {
    const priority: Record<string, number> = { searching: 0, connecting: 1, pending: 2, complete: 3, error: 4 };
